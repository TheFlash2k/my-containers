...
EMULATOR="qemu-arm"
DEBUG=
[ ! -z QEMU_GDB_DEBUG ] && DEBUG=(-g $QEMU_GDB_PORT)
...
if [ "$BASE" == "socat" ]; then
    rm -f /opt/ynetd
    $EMULATOR $DEBUG -L "$LIBRARY_PATH" su $RUN_AS -c "/opt/socat tcp-l:$PORT,reuseaddr,fork, EXEC:\"/app/$CHAL_NAME\",stderr | tee -a $LOG_FILE"
else
    rm -f /opt/socat
    # -lt => cpu time in seconds. Keeps connection opened for max 10 seconds.
    # -se => stderr to redirect to socket
    $EMULATOR $DEBUG -L "$LIBRARY_PATH" /opt/ynetd -lt 1 -p $PORT -u $RUN_AS -se y -d $START_DIR "/app/$CHAL_NAME" | tee -a $LOG_FILE
fi